...
-- If you've got a left apply the function to the left and vica versa
...
-- Put an f into a Fix f
inject :: (sub :<: f) => sub (Fix f) -> Fix f
inject = Fix . inj
-- Put an f into a constant function to a Fix f
finject :: (sub :<: f) => sub (Fix f) -> b -> Fix f
